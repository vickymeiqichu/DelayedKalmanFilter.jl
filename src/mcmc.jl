...
function kalman_filter_mh(protein_observations,model_parameters,measurement_variance,number_of_samples)
    ...
    @model kalman_model(protein_observations) = begin
           ...
           # model_parameters[3] ~ Determin(log(2)/30) # Uniform(log(2)/200,log(2)/10) #mRNA_degradation_rate
           # model_parameters[4] ~ Determin(log(2)/90) # Uniform(log(2)/200,log(2)/10) #protein_degradation_rate
