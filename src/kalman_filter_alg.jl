...
function kalman_filter_state_space_initialisation(protein_at_observations,model_parameters,measurement_variance = 10)
    ...
    observation_transform = [0.0 1.0]
    predicted_observation_distributions = Matrix{eltype(model_parameters)}(undef,number_of_observations,3);#Array{Normal{Float64}}(undef, states.number_of_observations)
    predicted_observation_distributions[1,1] = 0.0
...
function kalman_prediction_step(state_space_mean,
    ...
    # this is P(t,t)
    current_covariance_matrix = Matrix{eltype(model_parameters)}(undef,(2,2));#zeros((2,2))
    # this is P(t-tau,t) in page 5 of the supplementary material of Calderazzo et. al.
    covariance_matrix_past_to_now = Matrix{eltype(model_parameters)}(undef,(2,2));#zeros((2,2))
    # this is P(t,t-tau) in page 5 of the supplementary material of Calderazzo et. al.
    covariance_matrix_now_to_past = Matrix{eltype(model_parameters)}(undef,(2,2));#zeros((2,2))
    # This corresponds to P(s,t) in the Calderazzo paper
    covariance_matrix_intermediate_to_current = Matrix{eltype(model_parameters)}(undef,(2,2));#zeros((2,2))
    # This corresponds to P(s,t-tau)
    covariance_matrix_intermediate_to_past = Matrix{eltype(model_parameters)}(undef,(2,2));#zeros((2,2))
    # This corresponds to P(s,t+delta t)
    covariance_matrix_intermediate_to_next = Matrix{eltype(model_parameters)}(undef,(2,2));#zeros((2,2))
    ...
    for next_time_index in (current_number_of_states + 1):(current_number_of_states + number_of_hidden_states)
        ...
        derivative_of_mean = [-mRNA_degradation_rate 0.0;
                              ...
                              [basal_transcription_rate*hill_function_value, 0.0]
        ...
        # P(t+Deltat,t+Deltat)
        # if next_time_index == 40
        #     println(typeof(current_covariance_matrix))
        #     println(typeof(discretisation_time_step))
        #     println(typeof(derivative_of_variance))
        # end
